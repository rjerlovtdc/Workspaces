(function (global, factory) {
...
}(this, (function (exports, Mousetrap) {
	'use strict';
	...
	const DEFAULT_ORIGINS = [
		...
		'https://ciagent.puzzel.com',
		'https://testagent.puzzel.com',
		'https://qaagent.puzzel.com',
		'https://telenor-se.puzzel.com',
		'https://agent.telenor.se',
		'https://devazagent.puzzel.com'
	];

	const CALL_TIMEOUT = 20000;

	class Api {
		constructor(port, config, { receiver: customReceiver }) {
			this._id = 1000;
			this._calls = new Map();
			this._port = port;
			this._config = config;
	...
	function connect(origins = DEFAULT_ORIGINS, options = {}) {
		return new Promise((resolve, reject) => {
			...
			const handshake = message => {
				...
				if (origins.indexOf(message.origin) >= 0) {
					resolve(new Api(message.ports[0], message.data, options));
				} else {
					reject(new Error('Connection to host rejected: The origin doesn\'t match.'));
				}
			};

			window.addEventListener('message', handshake, { once: true });
			origins.forEach(origin => window.parent.postMessage(WIDGET_API_PROTOCOL_VERSION, origin));
		});
	}
